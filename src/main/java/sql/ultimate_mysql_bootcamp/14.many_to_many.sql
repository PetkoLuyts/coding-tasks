...
SELECT title, ROUND(AVG(rating), 2) AS avg_rating FROM series AS s
JOIN reviews AS r
ON s.id = r.series_id
GROUP BY s.title
...
SELECT first_name, last_name, rating FROM reviewers AS r
JOIN reviews AS rr
ON r.id = rr.reviewer_id;

-- TV Series Challenge #4
SELECT title AS unreviewed_series FROM series AS s
